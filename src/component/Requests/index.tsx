import {View, FlatList, Text, Image, Pressable, ActivityIndicator, Alert} from 'react-native';
import {useEffect, useState} from 'react';
import RequestComp from '../RequestComp';
import { useAuthContext } from 'context/use-auth-context';
import { useAppDispatch, useAppSelector } from '../../redux/hooks';
import { callUpdateRequest, updateRequestClear, updateRequestInfo } from 'routes/Dashboard/slice/UpdateRequest';
...
const {isUpdateRequestLoading, updateRequestSuccess, updateRequestError} = useAppSelector(updateRequestInfo)
...
const getHistoryForUser = () => {
  ...
  const acceptRequest = (id) => {
    ...
    sendNotificationByUserId(id?.donation?.userId,"Request accepted","Donation request for "+id?.deliveryTime+" is accepted by "+profileSuccess?.organizationName);
  ...
  const rejectRequest = (id) => {
    ...
    sendNotificationByUserId(id?.donation?.userId,"Request rejected",profileSuccess?.organizationName+" rejected the donation request");
    dispatch(callUpdateRequest(payload));
